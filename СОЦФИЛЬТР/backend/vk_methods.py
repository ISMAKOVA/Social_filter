...
def get_posts_comment(id, post_id, access_token, sort,count=10):
    try:
        session = vk.Session(access_token=access_token)
        vk_api = vk.API(session, v="5.126")
        return vk_api.wall.getComments(owner_id=id, post_id=post_id, count=str(count), sort=str(sort), preview_length=0)
    except vk.exceptions.VkAPIError as e:
        return e.message
