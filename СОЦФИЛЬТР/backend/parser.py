...
def get_last_comment(domain, at):
    post_id_list = vk_methods.get_posts(domain, 3, at)
    group_id = post_id_list['items'][0]['owner_id']
    post_id = post_id_list['items'][2]['id']
    comments = vk_methods.get_posts_comment(group_id, post_id, at, "desc", 100)
    com_texts = {}
    for j in range(len(comments['items'])):
        if 'text' in comments['items'][j]:
            com_texts[comments['items'][j]['from_id']] = re.sub('^\s+|\n|\r|\s+$', '', comments['items'][j]['text'])
    return com_texts
